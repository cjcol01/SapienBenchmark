...
def home():
    ...
    if current_user.is_authenticated:
        ...
        avg_score_reaction_query = db.session.query(
            db.func.avg(PlaySession.score)
        ).filter(
        ...
        avg_score_typing_query = db.session.query(
            db.func.avg(PlaySession.score)
        ).filter(
            PlaySession.user_id == current_user.id,
        ...
        avg_score_verbal_memory_query = db.session.query(
            db.func.avg(PlaySession.score)
        ).filter(
            PlaySession.user_id == current_user.id,
        ...
        reaction_percentage = calculate_percentile(
            avg_reaction, GAME_ID_FOR_REACTION, 1
        )
        ...
        verb_mem_percentage = calculate_percentile(
            avg_verbal_memory, GAME_ID_FOR_VERBAL_MEMORY, 4
        )
        ...
        return render_template(
            ...
            avg_reaction=str(avg_reaction),
            reaction_percentage=reaction_percentage,
            avg_aim=str(avg_aim),
            aim_percentage=aim_percentage,
            avg_typing=str(avg_typing),
            typing_percentage=typing_percentage,
            avg_verbal_memory=str(avg_verbal_memory),
            verb_mem_percentage=verb_mem_percentage,
        )
...
def calculate_percentile(user_score, game_id, game_id2):
    ...
    all_scores = (
        PlaySession.query.with_entities(PlaySession.score)
...
def submit_verbal_memory_score():
    ...
    play_session = PlaySession(
        user_id=current_user.id,
        game_id=GAME_ID_FOR_VERBAL_MEMORY,
        score=score,
        date_played=datetime.utcnow(),
    )
    db.session.add(play_session)
...
def update_user_game():
    ...
    play_data = (
        ...
        .group_by(PlaySession.user_id, PlaySession.game_id)
        .all()
    )
